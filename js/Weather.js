...
(function() {
    if (!window.jQuery) {
        ...
        script.onload = function() {
            // jQuery 加载完成后，执行获取位置和天气的函数
            fetchLocationAndWeather();
        };
        document.head.appendChild(script);
    } else {
        // jQuery 已加载，直接执行获取位置和天气的函数
        fetchLocationAndWeather();
    }
})();
...
function showWeather(weatherType) {
    ...
    if (weatherType === 'cloud') {
        ...
        container.innerHTML = '<div class="cloud_sun"><div class="sun"></div></div>';
...
async function getWeather(city) {
    const apiKey = '3f3c1b4a1586ffe0a7291c4556ad9f5f'; // OpenWeatherMap API 密钥
